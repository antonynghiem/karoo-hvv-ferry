...
class FerryDataType(
    ...
    karooSystemService: KarooSystemService? = null
    ...
    // Cache is now managed by FerryRepository (5 minute TTL)
    ...
    // Battery optimization: Reuse injected singleton if provided, else create own instance
    private val karooSystem: KarooSystemService = karooSystemService ?: KarooSystemService(context)
    // If an already-connected service was injected, mark as connected immediately
    @Volatile private var isKarooConnected = karooSystemService != null
    ...
    override fun startStream(emitter: Emitter<StreamState>) {
        ...
        streamJob = scope.launch {
            emitter.onNext(StreamState.Searching)
            var lastStreamingState: StreamState.Streaming? = null
            ...
            while (isActive) {
                ...
                // Reset state when stop changes so backoff doesn't delay the first fetch
                val currentStopId = config.manualStopId
                if (currentStopId != lastStopId) {
                    lastStopId = currentStopId
                    failureCount = 0
                    lastStreamingState = null
                    ...
                    if (shouldUpdate(config)) {
                        when (val result = updateFerryData(config)) {
                                ...
                                if (result.departure != null) {
                                    ...
                                    val state = StreamState.Streaming(
                                        dataPoint = DataPoint(
                                            ...
                                            values = mapOf(DataType.Field.SINGLE to minutesUntil)
                                        )
                                    )
                                    ...
                                    // Valid response but no departures — keep last state if we had one
                                    if (lastStreamingState == null) emitter.onNext(StreamState.Searching)
                            ...
                            is UpdateResult.Error -> {
                                failureCount++
                                // Keep last streaming state on error so overlay doesn't reappear
                                if (lastStreamingState == null) emitter.onNext(StreamState.Searching)
    ...
    override fun startView(context: Context, config: ViewConfig, emitter: ViewEmitter) {
        ...
        // Hide the Karoo OS "Next Ferry Departure" header label
        ...
        viewJob = scope.launch {
            var lastSuccessfulDepartures: List<dev.antonyng.hvvferry.data.models.Departure>? = null
            var lastViewStopId: String? = null

            while (isActive) {
                // Cache config once per cycle
                val ferryConfig = preferencesManager.getConfig()

                // Reset last departures when stop changes
                val currentStopId = ferryConfig.manualStopId
                if (currentStopId != lastViewStopId) {
                    lastViewStopId = currentStopId
                    lastSuccessfulDepartures = null
                }
                ...
                try {
                    if (shouldUpdate(ferryConfig)) {
                        ...
                        if (stop != null) {
                            val maxDepartures = 10
                            val result = repository.getDepartures(stop, maxDepartures)
                            val departures = result.getOrNull()

                            if (departures != null) {
                                // Success — update last known good data and refresh view
                                lastSuccessfulDepartures = departures
                            ...
                            } else if (lastSuccessfulDepartures != null) {
                                // Network error but we have cached data — keep showing it
                                Timber.d("⚠️ Fetch failed, keeping last known departures")
                            }
                            // else: first fetch failed, nothing to show yet
                        }
                    }
                } catch (e: Exception) {
    ...
    private fun startLocationUpdates() {
        ...
        if (isKarooConnected) {
            ...
            // Connect first (handles the case where FerryDataType created its own karooSystem)
    ...
    private suspend fun getActiveStop(config: FerryConfig): FerryStop? {
        ...
        // Use manual stop — try cache first, fall back to constructing from saved config
        val stopId = config.manualStopId ?: return null
        val cached = repository.getStopById(stopId)
        if (cached != null) return cached

        // Stop not in local DB yet (e.g. quick-pick selected before any GPS/search cache).
        // Build a minimal FerryStop from the saved config so departure lookup can proceed.
        val name = config.manualStopName ?: stopId
        Timber.d("📍 Manual stop not in cache, using config values: $name ($stopId)")
        return dev.antonyng.hvvferry.data.models.FerryStop(
            stationId = stopId,
            name = name,
            latitude = 0.0,
            longitude = 0.0,
    ...
    private suspend fun updateFerryData(config: FerryConfig): UpdateResult {
        ...
        // Repository handles caching (5 min TTL) — just call getDepartures
        val result = repository.getDepartures(stop)

        return result.fold(
            onSuccess = { departures ->
                Timber.d("📡 Fetched ${departures.size} departures for ${stop.name}")
